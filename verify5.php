...
if(count($_POST)>0) {
	...
	if(!isset($message)) {
		...
		if($count==0 && $count2==0) {
			$query = "INSERT INTO registered_users (user_name, password, email, token) VALUES
			('" . $_POST["username"] . "', '" . $hashedPassword . "', '" . $_POST["email"] . "', '" . $token . "')";
			...
			if(!empty($current_id)) {
				...
				$actual_link = $myBase . "/activate.php?token=" . $token;

				// Setting the email content
		    $mail->IsHTML(true);
		    $mail->Subject = "Activate Cardtraders Account";
		    $mail->Body = "Hello " . $username .",<br><br> Thank you for signing up with Cardtraders. <br><br> Click this link to activate your account: <a href='" . $actual_link . "'>" . $actual_link . "</a><br><br> All the best,<br><br> Cardtraders Team";
